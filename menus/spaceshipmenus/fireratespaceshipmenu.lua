local FirerateSpaceshipMenu = {}
FirerateSpaceshipMenu.__index = FirerateSpaceshipMenu
local AudioManager = require("audio/audiomanager")
local soundManager = AudioManager.new()
function FirerateSpaceshipMenu.new()
    local self = setmetatable({}, FirerateSpaceshipMenu)
...
function FirerateSpaceshipMenu:draw()
...
function FirerateSpaceshipMenu:mousepressed(x,y,b,s)
    if b==1 then
        for i,v in ipairs(self.buttons) do
            local a = false
            local x1, x2, x3, x4
            x1 = Data.player.money
            x2 = Data.firerate.lvl2.current
            x3 = Data.firerate.lvl3.current
            x4 = Data.firerate.lvl4.current
            if x>v.x and x<v.x+v.w and y>v.y and y<v.y+v.h then
                ...
                elseif v.text=="up.lvl2" then
                    if x1 >= Data.powerscost.firerate.lvl1 and x2 == false then
                        soundManager:playSound(1)
                        Data.firerate.lvl2.current = true
                        Data.firerate.lvl = 2
                        Data.player.money = Data.player.money - Data.powerscost.firerate.lvl1
                        a = true
                    end 
                elseif v.text=="up.lvl3" then
                    if x1 >= Data.powerscost.firerate.lvl2 and x3 == false and Data.firerate.lvl2.current then
                        soundManager:playSound(1)
                        Data.firerate.lvl3.current = true
                        Data.firerate.lvl = 3
                        Data.player.money = Data.player.money - Data.powerscost.firerate.lvl2
                        a = true
                    end 
                elseif v.text=="up.lvl4" then
                    if x1 >= Data.powerscost.firerate.lvl3 and x4 == false and Data.firerate.lvl3.current then
                        soundManager:playSound(1)
                        Data.firerate.lvl4.current = true
                        Data.firerate.lvl = 4
                        Data.player.money = Data.player.money - Data.powerscost.firerate.lvl3
                        a = true
                    end 
                end
            end
