local BombSpaceshipMenu = {}
BombSpaceshipMenu.__index = BombSpaceshipMenu
local AudioManager = require("audio/audiomanager")
local soundManager = AudioManager.new()
function BombSpaceshipMenu.new()
    local self = setmetatable({}, BombSpaceshipMenu)
...
function BombSpaceshipMenu:draw()
...
function BombSpaceshipMenu:mousepressed(x,y,b,s)
    if b==1 then
        for i,v in ipairs(self.buttons) do
            local a = false
            local x1, x2, x3, x4
            x1 = Data.player.money
            x2 = Data.bomb.lvl2.current
            x3 = Data.bomb.lvl3.current
            x4 = Data.bomb.lvl4.current
            if x>v.x and x<v.x+v.w and y>v.y and y<v.y+v.h then
                ...
                elseif v.text=="up.lvl2" then
                    if x1 >= Data.powerscost.bomb.lvl1 and x2 == false then
                        soundManager:playSound(1)
                        Data.bomb.lvl2.current = true
                        Data.bomb.lvl = 2
                        Data.player.money = Data.player.money - Data.powerscost.bomb.lvl1
                        a = true
                    end 
                elseif v.text=="up.lvl3" then
                    if x1 >= Data.powerscost.bomb.lvl2 and x3 == false and Data.bomb.lvl2.current then
                        soundManager:playSound(1)
                        Data.bomb.lvl3.current = true
                        Data.bomb.lvl = 3
                        Data.player.money = Data.player.money - Data.powerscost.bomb.lvl2
                        a = true
                    end 
                elseif v.text=="up.lvl4" then
                    if x1 >= Data.powerscost.bomb.lvl3 and x4 == false and Data.bomb.lvl3.current then
                        soundManager:playSound(1)
                        Data.bomb.lvl4.current = true
                        Data.bomb.lvl = 4
                        Data.player.money = Data.player.money - Data.powerscost.bomb.lvl3
                        a = true
                    end 
                end
            end
