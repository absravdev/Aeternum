local LaserSpaceshipMenu = {}
LaserSpaceshipMenu.__index = LaserSpaceshipMenu
...
function LaserSpaceshipMenu:draw()
...
function LaserSpaceshipMenu:mousepressed(x,y,b,s)
    if b==1 then
        for i,v in ipairs(self.buttons) do
            local a = false
            local x1, x2, x3, x4
            x1 = Data.player.money
            x2 = Data.gun.lvl2.current
            x3 = Data.gun.lvl3.current
            x4 = Data.gun.lvl4.current
            if x>v.x and x<v.x+v.w and y>v.y and y<v.y+v.h then
                print("Has hecho clic en el "..v.text)
                if v.text=="back" then
                    Data.currentState = "mainspaceshipmenu"
                elseif v.text=="up.lvl2" then
                    if x1 >= Data.powerscost.gun.lvl1 and x2 == false then
                        soundManager:playSound(1)
                        Data.gun.lvl2.current = true
                        Data.gun.lvl = 2
                        Data.player.money = Data.player.money - Data.powerscost.gun.lvl1
                        a = true
                    end 
                elseif v.text=="up.lvl3" then
                    if x1 >= Data.powerscost.gun.lvl2 and x3 == false and Data.gun.lvl2.current then
                        soundManager:playSound(1)
                        Data.gun.lvl3.current = true
                        Data.gun.lvl = 3
                        Data.player.money = Data.player.money - Data.powerscost.gun.lvl2
                        a = true
                    end 
                elseif v.text=="up.lvl4" then
                    if x1 >= Data.powerscost.gun.lvl3 and x4 == false and Data.gun.lvl3.current then
                        soundManager:playSound(1)
                        Data.gun.lvl4.current = true
                        Data.gun.lvl = 4
                        Data.player.money = Data.player.money - Data.powerscost.gun.lvl3
                        a = true
                    end 
                end
            end
