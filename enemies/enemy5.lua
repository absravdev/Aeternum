...
local Enemy5Bullets = require("bullets/enemy5bullets") 
function Enemy5.new(maxTime)
  local self = setmetatable({}, Enemy5)
  self.maxTime = maxTime
  self.timer = self.maxTime
  return self
end
function Enemy5:SpawnType1(zombies, speed, fireRate)
   local zombie = {}
   zombie.x = 0
   zombie.y = 0
   zombie.speed = speed
...
function Enemy5:fireBullet(zombie, player, bulletSpeed)
  local angle = math.atan2(player.y - zombie.y, player.x - zombie.x)
  local bullet = Enemy5Bullets:new(zombie.x, zombie.y, angle, bulletSpeed)
  table.insert(Enemy5Bullets, bullet) -- Añade la bala a la lista global de balas
end
function Enemy5:update(zombies, player, playerBullets, speed, fireRate, bulletSpeed, dt)
  self.timer = self.timer - dt
  if self.timer <= 0 then 
        Enemy5:SpawnType1(zombies, speed, fireRate)
    ...
    if z.dead == true then
      ...
      table.remove(zombies, i)
       ...
       if z.fireTimer <= 0 then
          Enemy5:fireBullet(z, player, bulletSpeed)
           z.fireTimer = z.fireRate
       end
    end
  end
  for i=#Enemy5Bullets,1,-1 do
    local b = Enemy5Bullets[i]
    b:update(player, dt)
...
function Enemy5:draw(zombies, sprite)
  ...
  for _,b in ipairs(Enemy5Bullets) do
      b:draw()
  end
end
return Enemy5