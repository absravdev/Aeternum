...
local Enemy6Bullets = require("bullets/enemy6bullets") 
function Enemy6.new(maxTime)
  local self = setmetatable({}, Enemy6)
  self.maxTime = maxTime
  self.timer = self.maxTime
  return self
end
function Enemy6:SpawnType1(zombies, speed, fireRate)
   local zombie = {}
   zombie.x = 0
   zombie.y = 0
   zombie.speed = speed
...
function Enemy6:fireBullet(zombie, player, bulletSpeed)
  local angle = math.atan2(player.y - zombie.y, player.x - zombie.x)
  local bullet = Enemy6Bullets:new(zombie.x, zombie.y, angle, bulletSpeed)
  table.insert(Enemy6Bullets, bullet)
end
function Enemy6:update(zombies, player, playerBullets, speed, fireRate, bulletSpeed, dt)
  self.timer = self.timer - dt
  if self.timer <= 0 then 
        Enemy6:SpawnType1(zombies, speed, fireRate)
        self.maxTime = 1 * self.maxTime
        self.timer = self.maxTime
  end 
  for i=#zombies,1,-1 do
    local z = zombies[i]
    if Collisions:distanceBetween(player.x, player.y, z.x, z.y) > 800 then
      if Data.lvl == 1 then
    ...
    if z.dead == true then
      ...
      table.remove(zombies, i)
       ...
       if z.fireTimer <= 0 then
          Enemy6:fireBullet(z, player, bulletSpeed)
           z.fireTimer = z.fireRate
       end
    end
  end
  for i=#Enemy6Bullets,1,-1 do
    local b = Enemy6Bullets[i]
    b:update(player, dt)
...
function Enemy6:draw(zombies, sprite)
  ...
  for _,b in ipairs(Enemy6Bullets) do
      b:draw()
  end
end
return Enemy6