...
function Enemy1.new(maxTime)
  local self = setmetatable({}, Enemy1)
  self.maxTime = maxTime
  self.timer = self.maxTime
  return self
end
function Enemy1:SpawnType1(zombies, speed)
   local zombie = {}
   zombie.x = 0
   zombie.y = 0
   zombie.speed = speed
...
function Enemy1:update(zombies, speed, dt)
  self.timer = self.timer - dt
  if self.timer <= 0 then 
        Enemy1:SpawnType1(zombies, speed)
...
function Enemy1:draw(zombies, sprite)
