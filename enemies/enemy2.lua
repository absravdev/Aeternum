...
local Enemy2Bullets = require("bullets/enemy2bullets") 
function Enemy2.new(maxTime)
  local self = setmetatable({}, Enemy2)
  self.maxTime = maxTime
  self.timer = self.maxTime
  return self
end
function Enemy2:SpawnType1(zombies, speed, fireRate)
   local zombie = {}
   zombie.x = 0
   zombie.y = 0
   zombie.speed = speed
...
function Enemy2:fireBullet(zombie, player, bulletSpeed)
  local angle = math.atan2(player.y - zombie.y, player.x - zombie.x)
  local bullet = Enemy2Bullets:new(zombie.x, zombie.y, angle, bulletSpeed)
  table.insert(Enemy2Bullets, bullet) -- Añade la bala a la lista global de balas
end
function Enemy2:update(zombies, player, playerBullets, speed, fireRate, bulletSpeed, dt)
  self.timer = self.timer - dt
  if self.timer <= 0 then 
        Enemy2:SpawnType1(zombies, speed, fireRate)
        self.maxTime = 1 * self.maxTime
        self.timer = self.maxTime
  end 
  for i=#zombies,1,-1 do
    local z = zombies[i]
    -- Aquí está la lógica del movimiento del enemigo
      if Data.lvl == 1 then
    ...
    if z.dead == true then
      ...
      table.remove(zombies, i)
       ...
       if z.fireTimer <= 0 then
           Enemy2:fireBullet(z, player, bulletSpeed)
           z.fireTimer = z.fireRate
       end
    end
  end
  for i=#Enemy2Bullets,1,-1 do
    local b = Enemy2Bullets[i]
    b:update(player, dt)
...
function Enemy2:draw(zombies, sprite)
  ...
  for _,b in ipairs(Enemy2Bullets) do
      b:draw()
  end
end
return Enemy2
